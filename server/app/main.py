...
from server.app.api.v1 import api_router as api_v1_router
...
async def lifespan(app: FastAPI):
    ...
    if mlmd_state.query:
        ...
        await mlmd_state.update_global_exe_dict(None)
        # loaded artifact ids into memory
        await mlmd_state.update_global_art_dict(None)
    # Start background scheduler task
    app.state.scheduler_task = asyncio.create_task(schedule_runner(sync_metadata))
    yield
    ...
    if task:
        task.cancel()
    mlmd_state.dict_of_art_ids.clear()
...
print("Local addresses= ", mlmd_state.LOCAL_ADDRESSES)
...
async def mlmd_push(info: MLMDPushRequest):
    ...
    if pipeline_name not in mlmd_state.pipeline_locks:    # create lock object for pipeline if it doesn't exists in lock
        mlmd_state.pipeline_locks[pipeline_name] = asyncio.Lock()
    pipeline_lock = mlmd_state.pipeline_locks[pipeline_name]
    mlmd_state.lock_counts[pipeline_name] += 1 # increment lock count by 1 if pipeline going to enter inside lock section
    async with pipeline_lock:
        try:
            ...
            if status != "exists":
                # async function
                await mlmd_state.update_global_exe_dict(pipeline_name)
                await mlmd_state.update_global_art_dict(pipeline_name)
        finally:
            mlmd_state.lock_counts[pipeline_name] -= 1  # Decrement the reference count after lock released
            if mlmd_state.lock_counts[pipeline_name] == 0:   #if lock_counts of pipeline is zero means lock is release from it
                del mlmd_state.pipeline_locks[pipeline_name]  # Remove the lock if it's no longer needed
                del mlmd_state.lock_counts[pipeline_name]
...
async def mlmd_pull(info: MLMDPullRequest):
    ...
    await mlmd_state.check_mlmd_file_exists()
    if pipeline_name:
        # checks if pipeline exists
        await mlmd_state.check_pipeline_exists(pipeline_name)
        #json_payload values can be json data, none or no_exec_id.
        json_payload= await async_api(get_mlmd_from_server, query, pipeline_name, exec_uuid, last_sync_time, mlmd_state.dict_of_exe_ids)
    else:
...
async def execution_lineage(request: Request, uuid: str, pipeline_name: str):
    ...
    # checks if pipeline exists
    await mlmd_state.check_pipeline_exists(pipeline_name)
    response = await async_api(query_execution_lineage_d3tree, query, pipeline_name, mlmd_state.dict_of_exe_ids, uuid)
    return response
...
async def artifact_lineage_tangled(request: Request, pipeline_name: str) -> Optional[List[List[Dict[str, Any]]]]:
    ...
    # checks if pipeline exists
    await mlmd_state.check_pipeline_exists(pipeline_name)
    response = await async_api(query_artifact_lineage_d3tree, query, pipeline_name, mlmd_state.dict_of_art_ids)
    return response
...
async def artifact_execution_lineage(request: Request, pipeline_name: str):
    ...
    # checks if pipeline exists
    await mlmd_state.check_pipeline_exists(pipeline_name)
    response = await async_api(query_visualization_artifact_execution, query, pipeline_name, mlmd_state.dict_of_art_ids, mlmd_state.dict_of_exe_ids)
    return response
...
async def list_of_executions(request: Request, pipeline_name: str):
    ...
    # checks if pipeline exists
    await mlmd_state.check_pipeline_exists(pipeline_name)
    response = await async_api(executions_list, query, pipeline_name, mlmd_state.dict_of_exe_ids)
    return response
...
async def register_server(request: ServerRegistrationRequest, db: AsyncSession = Depends(get_db)):
    try:
        ...
        if server in mlmd_state.LOCAL_ADDRESSES:
